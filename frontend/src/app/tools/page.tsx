...
export default function ToolsExplorerPage() {
  ...
  async function executeTool() {
    ...
    try {
      let parsedPayload: ToolPayload = {};
      try {
        const parsed = JSON.parse(payloadText);
        if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
          throw new Error("payload must be a JSON object");
        }
        parsedPayload = parsed as ToolPayload;
      } catch {
      ...
      switch (activeToolId) {
        case "lookupCustomer": res = await api.tools.lookupCustomer(asString(parsedPayload.customer_id)); break;
        case "getInvoiceHistory": res = await api.tools.getInvoiceHistory(asString(parsedPayload.customer_id), asNumber(parsedPayload.months, 3)); break;
        case "checkDuplicateCharge": res = await api.tools.checkDuplicateCharge(asString(parsedPayload.customer_id), asNumber(parsedPayload.lookback_days, 30)); break;
        case "checkOutageStatus": res = await api.tools.checkOutageStatus(asString(parsedPayload.location), optionalString(parsedPayload.customer_id)); break;
        case "runRouterDiagnostic": res = await api.tools.runRouterDiagnostic(asString(parsedPayload.customer_id)); break;
        case "retrievePolicy": res = await api.tools.retrievePolicy(asString(parsedPayload.policy_name), optionalString(parsedPayload.query), asNumber(parsedPayload.top_k, 3)); break;
        case "applyCredit": res = await api.tools.applyCredit(parsedPayload as unknown as ApplyCreditRequest); break;
        case "createTicket": res = await api.tools.createTicket(parsedPayload as unknown as CreateTicketRequest); break;
        case "scheduleTechnician": res = await api.tools.scheduleTechnician(parsedPayload as unknown as ScheduleTechnicianRequest); break;
        case "changePlan": res = await api.tools.changePlan(parsedPayload as unknown as ChangePlanRequest); break;
        case "generateHandoffSummary": res = await api.tools.generateHandoffSummary(parsedPayload as unknown as HandoffSummaryRequest); break;
        case "generateContextCard": res = await api.tools.generateContextCard(parsedPayload as unknown as ContextCardRequest); break;
        case "generateOpeningLine": res = await api.tools.generateOpeningLine(parsedPayload as unknown as OpeningLineRequest); break;
        case "generateAuditLog": res = await api.tools.generateAuditLog(parsedPayload as unknown as AuditLogRequest); break;
