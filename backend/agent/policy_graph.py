...
class PolicyNode:
    ...
    def __post_init__(self) -> None:
        ...
        condition_type = self.condition_type.strip() if isinstance(self.condition_type, str) else self.condition_type
        action = self.action.strip() if isinstance(self.action, str) else self.action
        condition_args = dict(self.condition_args or {})
        edges = {str(result).strip(): str(target).strip() for result, target in dict(self.edges or {}).items()}
        ...
        if any(not result or not target for result, target in edges.items()):
            raise ValueError("edges must map non-empty condition results to non-empty node IDs")

        is_leaf = action is not None
        if is_leaf and (condition_type is not None or edges):
            raise ValueError("leaf action nodes must not define condition_type or edges")
    ...
    def next_node(self, condition_result: str) -> str:
        ...
        try:
            ...
            raise KeyError(f"node {self.node_id} has no edge for result {result!r}") from exc
...
class PolicyDAG:
    ...
    def __post_init__(self) -> None:
        ...
        if start_node not in nodes:
            raise ValueError(f"start_node {start_node!r} is not present in nodes")

        for node_id, node in nodes.items():
            if node_id != node.node_id:
                raise ValueError(f"node map key {node_id!r} must match PolicyNode.node_id {node.node_id!r}")
            for target in node.edges.values():
                if target not in nodes:
                    raise ValueError(f"node {node.node_id} points to missing node {target!r}")
    ...
    def run(self, policy_name: str, context: dict[str, Any]) -> PolicyValidationResult:
        ...
        while True:
            if current_node_id in visited:
                raise ValueError(f"cycle detected at policy node {current_node_id}")
...
def build_service_credit_dag() -> PolicyDAG:
    nodes = {
        "check_outage_verified": PolicyNode(
            ...
            edges={"true": "check_outage_duration", "false": "manual_review_credit"},
        ),
        ...
        "check_prior_credit": PolicyNode(
            ...
            condition_type="tool_check",
            condition_args={"tool": "get_invoice_history", "field": "credit_this_cycle"},
            edges={"false": "auto_apply_credit", "true": "manual_review_credit"},
        ),
...
def build_duplicate_charge_refund_dag() -> PolicyDAG:
    nodes = {
        "check_duplicate_confirmed": PolicyNode(
            ...
            condition_type="tool_check",
            condition_args={"tool": "check_duplicate_charge", "field": "duplicate_confirmed"},
            edges={"true": "check_invoice_match", "false": "deny_duplicate_refund"},
        ),
        "check_invoice_match": PolicyNode(
            ...
            condition_type="tool_check",
            condition_args={"tool": "get_invoice_history", "field": "single_matching_invoice"},
            edges={"true": "check_refund_window", "false": "manual_refund_review"},
        ),
        "check_refund_window": PolicyNode(
            ...
            condition_type="threshold_check",
            condition_args={"field": "payment_age_days", "operator": "<=", "value": 30},
            edges={"true": "check_duplicate_amount", "false": "manual_refund_review"},
        ),
        "check_duplicate_amount": PolicyNode(
            ...
            condition_type="threshold_check",
            condition_args={"field": "duplicate_amount", "operator": "<=", "value": 500},
            edges={"true": "create_refund_review_ticket", "false": "manual_refund_review"},
        ),
        "create_refund_review_ticket": PolicyNode(
            ...
            args={"ticket_type": "duplicate_charge_refund_review", "allow_account_credit": True},
        ),
...
def build_cancellation_retention_dag() -> PolicyDAG:
    nodes = {
        "check_identity_status": PolicyNode(
            ...
            condition_type="tool_check",
            condition_args={"tool": "lookup_customer", "field": "identity_verified"},
        ...
        "check_open_issues": PolicyNode(
            ...
            edges={"true": "create_retention_ticket", "false": "check_churn_risk"},
        ),
        "check_churn_risk": PolicyNode(
            node_id="check_churn_risk",
            description="Escalate high-risk cancellation customers to retention.",
            condition_type="threshold_check",
            condition_args={"field": "churn_score", "operator": ">=", "value": 0.7},
            edges={"true": "handoff_retention", "false": "explain_cancellation_steps"},
        ),
...
def build_technician_dispatch_dag() -> PolicyDAG:
    nodes = {
        "check_account_active": PolicyNode(
            ...
            edges={"true": "check_router_diagnostic", "false": "manual_dispatch_review"},
        ),
        "check_router_diagnostic": PolicyNode(
            ...
            condition_type="tool_check",
            condition_args={"tool": "run_router_diagnostic", "field": "diagnostic_failure"},
            edges={"true": "check_outage_cleared", "false": "manual_dispatch_review"},
        ),
        "check_outage_cleared": PolicyNode(
            ...
            condition_type="tool_check",
            condition_args={"tool": "check_outage_status", "field": "outage_cleared"},
            edges={"true": "check_appointment_slot", "false": "manual_dispatch_review"},
        ),
        "check_appointment_slot": PolicyNode(
            ...
            edges={"true": "schedule_technician_visit", "false": "manual_dispatch_review"},
        ),
...
def build_plan_downgrade_dag() -> PolicyDAG:
    nodes = {
        "check_account_active": PolicyNode(
            ...
            condition_type="tool_check",
            condition_args={"tool": "lookup_customer", "field": "account_active"},
            edges={"true": "check_overdue_invoice", "false": "manual_plan_review"},
        ),
        "check_overdue_invoice": PolicyNode(
            ...
            edges={"false": "check_plan_available", "true": "manual_plan_review"},
        ),
        "check_plan_available": PolicyNode(
            ...
            edges={"true": "check_price_speed_confirmed", "false": "manual_plan_review"},
        ),
        "check_price_speed_confirmed": PolicyNode(
            ...
            edges={"true": "check_promo_lockin", "false": "manual_plan_review"},
        ),
        "check_promo_lockin": PolicyNode(
            ...
            edges={"false": "schedule_plan_downgrade", "true": "disclose_fee_and_schedule"},
        ),
        ...
        "disclose_fee_and_schedule": PolicyNode(
            ...
            action="change_plan",
            args={"change_type": "downgrade", "effective": "next_billing_cycle", "fee_disclosure_required": True},
        ),
...
def build_refund_exception_dag() -> PolicyDAG:
    nodes = {
        "check_refund_reason": PolicyNode(
            ...
            edges={"true": "check_payment_ownership", "false": "deny_refund_exception"},
        ),
        "check_payment_ownership": PolicyNode(
            ...
            edges={"true": "check_refund_window", "false": "manual_refund_exception_review"},
        ),
        "check_refund_window": PolicyNode(
            node_id="check_refund_window",
            ...
            condition_type="threshold_check",
            condition_args={"field": "payment_age_days", "operator": "<=", "value": 30},
            edges={"true": "check_refund_amount", "false": "manual_refund_exception_review"},
        ),
        "check_refund_amount": PolicyNode(
            ...
            condition_type="threshold_check",
            condition_args={"field": "refund_amount", "operator": "<=", "value": 500},
            edges={"true": "create_refund_review_ticket", "false": "manual_refund_exception_review"},
        ),
        "create_refund_review_ticket": PolicyNode(
            node_id="create_refund_review_ticket",
...
def evaluate_policy_condition(node: PolicyNode, context: dict[str, Any]) -> str:
    ...
    raise ValueError(f"node {node.node_id} has unsupported condition_type {node.condition_type!r}")
...
def _evaluate_threshold(node: PolicyNode, context: dict[str, Any]) -> bool:
    ...
    if operator not in THRESHOLD_OPERATORS:
        raise ValueError(f"threshold node {node.node_id} has unsupported operator {operator!r}")
    ...
    try:
        ...
        raise ValueError(f"threshold node {node.node_id} values must be numeric") from exc
...
def _condition_value(node: PolicyNode, context: dict[str, Any]) -> Any:
    ...
    if not field_name:
        raise ValueError(f"node {node.node_id} condition_args must include field")
    ...
    raise ValueError(f"context missing field {field_name!r} for node {node.node_id}")
