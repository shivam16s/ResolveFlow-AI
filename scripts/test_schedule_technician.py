...
def assert_schedules_technician_and_creates_ticket() -> None:
    ...
    if result["time_slot"] != "2026-05-24 10:00-13:00":
...
def assert_schedules_against_existing_ticket() -> None:
    ...
    with sqlite3.connect(db_path) as connection:
        connection.row_factory = sqlite3.Row
        row = connection.execute(
            "SELECT status, priority FROM tickets WHERE ticket_id = ?",
            ("TKT-EXISTING-001",),
        ).fetchone()
...
def assert_validates_inputs_and_ticket_ownership() -> None:
    ...
    bad_calls = (
        {"customer_id": "   ", "time_slot": "2026-05-24 10:00-13:00", "policy_context": VALID_DISPATCH_CONTEXT},
        {"customer_id": "CUST-1001", "time_slot": "   ", "policy_context": VALID_DISPATCH_CONTEXT},
        {"customer_id": "CUST-1001", "time_slot": "2026-05-24 10:00-13:00", "policy_context": []},
        {
            "customer_id": "CUST-1001",
            "time_slot": "2026-05-24 10:00-13:00",
            "policy_context": VALID_DISPATCH_CONTEXT,
            "policy_name": "   ",
        },
        {
